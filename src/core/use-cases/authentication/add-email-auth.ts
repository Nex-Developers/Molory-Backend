...
export default function makeAddEmailAuth ({
    ...
    return async function ({
        ...
        const user = await userDb.findFirst({ where: { id }, select: { email: true, password: true, firstName: true, lang: true }})
        ...
        await askToConfirmEmail({ email, token, firstName: user.firstName, lang: user.lang })
