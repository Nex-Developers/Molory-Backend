...
export default function makeAdd({
    ...
    return async ({
        ...
        // const { amount, status, walletId} = await prisma.transaction.findUnique({ where: { id }, select: { amount: true, status: true, wallet: { select: { lastName: true, firstName: true, email: true}}}})
        ...
        // await prisma.transaction.update({ where: { id }, data: { transactionId, status: newStatus }})
