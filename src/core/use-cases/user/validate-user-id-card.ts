...
export default function makeValidateUserIdCard({
    ...
    return async function ({
        ...
        if (response !== "validate") {
            ...
            if (user.driverLicenseStatus == 1) notifyUser({ id: userId, titleRef: {text: 'notification.driverActivated.title'}, messageRef:{ text: 'notification.driverActivated.message', params: { name: user.lastName}  }, cover: null,  data: { path: 'validate-id-card', id: userId.toString(), res:'INFOS'}, lang: 'fr', type: 'user' })
        }
        saveProfile(userId)
