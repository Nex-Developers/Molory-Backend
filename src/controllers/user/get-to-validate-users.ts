...
export default function makeGetToValidateUsersController({
    listToValidateUsers
}) {
    // use translations
    return async function getUsersController(request: IHttpRequest): Promise<IHttpResponse> {
        const reqLog: Log = {
            ...
            description: `${request.ref.lastName}  ${request.ref.firstName}  ${Action.READ} all to validate users`
        } 

        try {
            const lang = request.lang,
                body = request.body,
                data = await listToValidateUsers({...body})
